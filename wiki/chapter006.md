...
$$\begin{array}{l} \begin{array}{lll} p & {= \left\lbrack {2m\left( {p\prime\alpha - \frac{k}{2}x^{2}} \right)} \right\rbrack^{1/2}} & \\  & {= \sqrt{2p\prime\beta}\,\cos x\prime.} & {(6.28)} \\ \end{array} \\  \\ \end{array}$$
...
$$\begin{array}{lll} {E_{\mathit{\epsilon}_{1},W_{1}}^{\prime}E_{\mathit{\epsilon}_{2},W_{2}}^{\prime}I} & {= \left( {E_{\mathit{\epsilon}_{1},W_{1}}^{\prime}\left( {E_{\mathit{\epsilon}_{2},W_{2}}^{\prime}I} \right)} \right) \circ I} & \\  & {= \left( {E_{\mathit{\epsilon}_{2},W_{2}}^{\prime}I} \right) \circ \left( {E_{\mathit{\epsilon}_{1},W_{1}}^{\prime}I} \right),} & {(6.111)} \\ \end{array}$$
...
$$\begin{matrix} {\left\lbrack {A,B} \right\rbrack = AB - BA.} & {(6.149)} \\ \end{matrix}$$
The commutator is bilinear and satisfies the Jacobi identity
$$\begin{matrix} {\left\lbrack {A,\left\lbrack {B,C} \right\rbrack} \right\rbrack + \left\lbrack {B,\left\lbrack {C,A} \right\rbrack} \right\rbrack + \left\lbrack {C,\left\lbrack {A,B} \right\rbrack} \right\rbrack = 0,} & {(6.150)} \\ \end{matrix}$$
which is true for all /A/, /B/, and /C/.

We introduce a notation Δ/_{A}/ for the commutator with respect to the operator /A/:
$$\begin{matrix} {\Delta_{A}B = \left\lbrack {A,B} \right\rbrack.} & {(6.151)} \\ \end{matrix}$$
#page(452)

In terms of Δ the Jacobi identity is
$$\begin{matrix} {\left\lbrack {\Delta_{A},\Delta_{B}} \right\rbrack = \Delta_{\lbrack{A,B}\rbrack}.} & {(6.152)} \\ \end{matrix}$$
An important identity is
$$\begin{array}{lll} {e^{C}Ae^{- C}} & {= e^{\Delta C}A} & \\  & {= A + \left\lbrack {C,A} \right\rbrack + \frac{1}{2}\left\lbrack {C,\left\lbrack {C,A} \right\rbrack} \right\rbrack + \cdots.} & {(6.153)} \\ \end{array}$$
We can check this term by term.

We see that
$$\begin{matrix} {e^{C}A^{2}e^{- C} = e^{C}Ae^{- C}e^{C}Ae^{- C} = \left( {e^{C}Ae^{- C}} \right)^{2},} & {(6.154)} \\ \end{matrix}$$
using /e/^{−C}/e^{C}/ = /I/, the identity operator. Using the same trick, we find
$$\begin{matrix} {e^{C}A^{n}e^{- C} = \left( {e^{C}Ae^{- C}} \right)^{n}.} & {(6.155)} \\ \end{matrix}$$
More generally, if /f/ can be represented as a power series then
$$\begin{matrix} {e^{C}f\left( {A,B,\cdots} \right)e^{- C} = f\left( {e^{C}Ae^{- C},e^{C}Be^{- C},\cdots} \right).} & {(6.156)} \\ \end{matrix}$$
For instance, applying this to the exponential function yields
$$\begin{matrix} {e^{C}e^{A}e^{- C} = e^{e^{C}Ae^{- C}}.} & {(6.157)} \\ \end{matrix}$$
Using equation (#Eqn(chapter006,6.153,6.153)), we can rewrite this as
$$\begin{matrix} {e^{\Delta C}e^{A} = e^{e^{\Delta C}A}.} & {(6.158)} \\ \end{matrix}$$
...
$$\begin{matrix} {\left\lbrack {L_{W,}L_{W\prime}} \right\rbrack = - L_{\{{W,W\prime}\}}.} & {(6.159)} \\ \end{matrix}$$
...
Show
$$\begin{matrix} {\left\lbrack {L_{J_{x}},L_{J_{y}}} \right\rbrack + L_{J_{z}} = 0.} & {(6.160)} \\ \end{matrix}$$
...
$$\begin{matrix} {e^{A}e^{B} = e^{A + B + \frac{1}{2}{\lbrack{A,B}\rbrack} + \cdots}.} & {(6.161)} \\ \end{matrix}$$
### 6.6 Summary
...
$$\begin{matrix} {\delta_{2\pi}\left( t \right) = {\sum\limits_{n = - \infty}^{\infty}{\delta\left( {t - 2\pi n} \right),}}} & {(6.163)} \\ \end{matrix}$$
with representation as a Fourier series
$$\begin{matrix} {2\pi\delta_{2\pi}\left( t \right) = {\sum\limits_{n = - \infty}^{\infty}{\cos\left( {nt} \right).}}} & {(6.164)} \\ \end{matrix}$$
...
$$\begin{matrix} {H_{m}\left( {t,q,p} \right) = H_{0}\left( {t,q,p} \right) + 2\pi\delta_{2\pi}\left( {\Omega t} \right)H_{1}\left( {t,q,p} \right).} & {(6.165)} \\ \end{matrix}$$
...
$$\begin{matrix} {e^{\Delta t\, L_{H_{m}}}I = \left( {e^{\Delta t\, L_{H_{1}}}I} \right) \circ \left( {e^{\Delta t\, L_{H_{0}}}I} \right).} & {(6.166)} \\ \end{matrix}$$
...
$$\begin{matrix} {e^{\Delta tL_{H_{m}}}I = \left( {e^{{({\Delta t/2})}L_{H_{0}}}I} \right) \circ \left( {e^{\Delta tL_{H_{1}}}I} \right) \circ \left( {e^{{({\Delta t/2})}L_{H_{0}}}I} \right).} & {(6.167)} \\ \end{matrix}$$
...
$$\begin{array}{ll} {H_{0}\left( {t;x,y;p_{x},p_{y}} \right) = \left( {p_{x}^{2} + p_{y}^{2}} \right)/2 + \left( {x^{2} + y^{2}} \right)/2} & \\ {H_{1}\left( {t;x,y;p_{x},p_{y}} \right) = x^{2}y - y^{3}/3.} & {(6.168)} \\ \end{array}$$
...
#FootnoteEnd
#FootnoteRef(13)The Lie-transform procedure here is also defined to be an operator, just like Lie-derivative. #FootnoteEnd